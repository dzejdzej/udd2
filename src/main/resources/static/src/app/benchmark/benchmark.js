angular.module('ngBoilerplate.benchmark', ['ui.router', 'ngResource', 'base64', 'checklist-model'])
    ...
    .factory('BenchmarkImportService', function ($resource) {
        var baseUrl = '/LuceneAnalyzerTester/rest/benchmark-importer';
        return $resource(baseUrl, {},
            {
                importBenchmark: {
                    url: baseUrl + '/importBenchmark',
                    headers: {
    ...
    .controller("BenchmarkCtrl", function ($scope, $http, $state, role, IsLoged,BenchmarkImportService, benchmarks, directories, analyzers) {
        ...
        $scope.uploadFile = function () {
            ...
            formData.append('benchmarkId', $scope.selectedBenchmark.benchmark_id);

            // .add(formData).$promise.then(function () {
            BenchmarkImportService.importBenchmark(formData).$promise.then(function () {
            
                   alert("Benchmark data improted!");
                   window.location.reload();
                },function () {
                    alert('Error while importing benchmark data!');
                });
        };
